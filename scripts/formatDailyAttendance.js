function formatDataForAttendance(reportData) {
    let output = ``;
    output += `<html>
    <body>
    ...
    <tr>
            ...
            reportData[0]?.['statuses']?.forEach(statuses => {
                output += `
            <td style="width:47pt; height:54px; text-align:center; white-space:nowrap; border-bottom:1pt solid rgb(217,217,217); background-color:white; padding-top:1px; padding-right:1px; padding-left:1px; vertical-align:middle"
                class="x_elementToProof">
                <div style="text-align:center; white-space:nowrap; font-family:Aptos,Aptos_EmbeddedFont,Aptos_MSFontService,Calibri,Helvetica,sans-serif; font-size:12pt; color:rgb(0,0,0)"
                    class="x_elementToProof"><span style="font-weight:700">${statuses.date}</span></div>
            </td>`
            });
            output += `</tr>`
    reportData.forEach(attendee => {
        output += `<tr>
            ...
            attendee.statuses.forEach(status => {
                output += `<td style="width:62.667px; height:25px; text-align:center; white-space:nowrap; border-top:1pt solid rgb(217,217,217); border-bottom:0.5pt solid rgb(217,217,217); background-color:${status.statusColor} padding-top:1px; padding-right:1px; padding-left:1px; vertical-align:middle"
                class="x_elementToProof">
                <div style="text-align:center; white-space:nowrap; font-family:Aptos,Aptos_EmbeddedFont,Aptos_MSFontService,Calibri,Helvetica,sans-serif; font-size:12pt; color:rgb(0,0,0)"
                    class="x_elementToProof"><span style="font-weight:700">${status.status}</span></div>
            </td>`
            });
            output += `</tr>`
    });
